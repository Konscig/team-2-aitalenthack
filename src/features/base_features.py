...
def _report(features: pd.DataFrame, causality: dict[str, object], input_path: Path, outputs: list[Path]) -> str:
    ...
    lines += ["", "## Сводная статистика", "", statistics.to_markdown(), "", "## Примеры строк", "", features.groupby("corridor", sort=True).tail(1).to_markdown(index=False), "", "## Проверка причинности", "", f"Статус: **{causality['status']}**. Все признаки сопоставлены после обрезки исходной истории на дате T для `{causality['samples']}` детерминированно выбранных случайных пар дата/коридор (seed `{causality['seed']}`).", "", "Выходные файлы: " + ", ".join(f"`{path.as_posix()}`" for path in outputs) + ".", "", "В рамках этапа base features не создавались labels, межвалютные признаки, макроданные, календарное расширение или синтетические production-значения. Advanced и calendar feature-артефакты построены отдельными последующими этапами.", "", "**BASE FEATURES STATUS: PASS**", ""]
    return "\n".join(lines)
